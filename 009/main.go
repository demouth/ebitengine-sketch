...
func (g *Game) Update() error {
	g.count++
	if g.count%30 == 0 {
		addCircle(g.space, rand.Float64()*10+10, rand.Float64()*10-5, -screenHeight/2+20)
		addBox(g.space, rand.Float64()*10+10, rand.Float64()*20+10, rand.Float64()*10-5, -screenHeight/2+20)
	}
...
func main() {
	...
	sides := []cp.Vector{
		{X: -hwidth + 50, Y: -hheight}, {X: -hwidth, Y: hheight},
		{X: hwidth, Y: -hheight}, {X: hwidth - 20, Y: hheight},
		{X: -hwidth, Y: -hheight}, {X: hwidth, Y: -hheight},
		{X: -hwidth, Y: hheight - 100}, {X: hwidth, Y: hheight},
	}
