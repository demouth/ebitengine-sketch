...
func NewTextInput(opts ...TextInputOpt) *TextInput {
	t := &TextInput{
		...
		repeatDelay:    900 * time.Millisecond,
...
func (t *TextInput) idleState(newKeyOrCommand bool) textInputState {
	return func() (textInputState, bool) {
		...
		input.HandleInput(t.InputPoint())
		// // println(t.widget.Rect.Min.X, t.widget.Rect.Min.Y)
...
func (t *TextInput) drawTextAndCaret(screen *ebiten.Image, def DeferredRenderFunc) {
	...
	bc := input.BeforeCommit()
	if t.focused {
		inputStr = string([]rune(inputStr)[:t.cursorPosition]) + bc + string([]rune(inputStr)[t.cursorPosition:])
	}
	...
	if t.focused {
		bcl := len([]rune(bc))
		sub := string([]rune(inputStr)[:t.cursorPosition+bcl])
	...
	if len([]rune(inputStr)) > 0 {
		t.text.Label = inputStr
	} else {
		t.text.Label = t.placeholderText
	}
	if t.widget.Disabled || len([]rune(inputStr)) == 0 {
	...
	{
		if t.focused {
			color := color.NRGBA{200, 200, 200, 100}
			img := image.NewNineSliceColor(color)

			w := fontAdvance(bc, t.face)
			img.Draw(screen, w, t.widget.Rect.Dy(), func(opts *ebiten.DrawImageOptions) {
				sub := string([]rune(inputStr)[:t.cursorPosition])
				cx = fontAdvance(sub, t.face)
				p := t.widget.Rect.Min
				opts.GeoM.Translate(float64(p.X+t.padding.Right), float64(p.Y))
				opts.GeoM.Translate(float64(cx), float64(0))
			})
		}
	}
}
...
func (t *TextInput) InputPoint() img.Point {

	cx := fontAdvance(t.inputText, t.face)

	return img.Point{
		t.widget.Rect.Min.X + t.scrollOffset + cx + t.caret.Width + t.padding.Right,
		t.widget.Rect.Min.Y + t.widget.Rect.Dy() + t.padding.Dy(),
	}
}
